...
function analyzeTaddhita(word, context) {
    ...
    // Common taddhita affixes and their patterns (ordered by specificity/length) using shared constants
    const taddhita_patterns = SanskritWordLists.taddhitaPatterns;
...
function analyzeDeclension(word, context) {
    ...
    if (taddhita_analysis.has_taddhita) {
        const typically_indeclinable = SanskritWordLists.typicallyIndeclinableAffixes;
...
function isTypicallyIndeclinable(affix_type) {
    const indeclinable_affixes = SanskritWordLists.indeclinableAffixTypes;
