...
describe("Sutra 1.4.86: hīne", () => {
    ...
    test('IAST: Should apply when particle is "anu" and meaning is "hīna"', () => {
        const result = sutraModule.default('anu', { meaning: 'hīna' });
    ...
    for (let i = 0; i < 10; i++) {
        test(`IAST: Positive case ${i + 1}`, () => {
            const result = sutraModule.default('anu', { meaning: 'hīna', note: `test case ${i}` });
            expect(result.applies).toBe(true);
        });
    }

    // Positive Test Cases (Devanagari)
    test('Devanagari: Should apply when particle is "अनु" and meaning is "hīna"', () => {
        const result = sutraModule.default('अनु', { meaning: 'hīna' });
        expect(result.applies).toBe(true);
        expect(result.designation).toBe('karmapravacanīya');
    });
    ...
    for (let i = 0; i < 10; i++) {
        test(`Devanagari: Positive case ${i + 1}`, () => {
            const result = sutraModule.default('अनु', { meaning: 'hīna', note: `परीक्षणं ${i}` });
            expect(result.applies).toBe(true);
        });
    }
    ...
    test('Should not apply if meaning is not "hīna"', () => {
    ...
    for (let i = 0; i < 10; i++) {
        test(`Negative case ${i + 1}: Wrong particle`, () => {
            const result = sutraModule.default(`pari${i}`, { meaning: 'hīna' });
            expect(result.applies).toBe(false);
        });
    }
    ...
    test('Handles null particle', () => {
        const result = sutraModule.default(null, { meaning: 'hīna' });
    ...
    test('Handles empty string as particle', () => {
        const result = sutraModule.default('', { meaning: 'hīna' });
        expect(result.applies).toBe(false);
    });
});
