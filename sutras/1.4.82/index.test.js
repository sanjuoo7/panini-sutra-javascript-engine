...
describe('Sutra 1.4.82: vyavahitāśca', () => {
    ...
    test('IAST: Particle separated by one word', () => {
        ...
        const result = sutra(sentence, context);
    ...
    test('Handles invalid context (null)', () => {
        const sentence = "some sentence";
        const context = null;
        const result = sutra(sentence, context);
    ...
    test('Handles invalid context (non-object)', () => {
        const sentence = "some sentence";
    ...
    test('Handles missing particles array', () => {
        const sentence = "some sentence";
        const context = { isVedic: true, verb: 'verb' };
        const result = sutra(sentence, context);
        expect(result.error).toBe('Context must include particles array');
    });

    test('Handles empty particles array', () => {
        const sentence = "some sentence";
        const context = { isVedic: true, verb: 'verb', particles: [] };
        const result = sutra(sentence, context);
        expect(result.error).toBe('Context must include particles array');
    });
    ...
    test('Handles verb not found in sentence', () => {
        const sentence = "some other words";
        const context = { isVedic: true, verb: 'notfound', particles: ['particle'] };
        const result = sutra(sentence, context);
        expect(result.applies).toBe(false);
        expect(result.error).toContain("Verb 'notfound' not found in sentence");
    });

    test('Handles verb found by substring match', () => {
        const sentence = "verbcompound word particle";
        const context = { isVedic: true, verb: 'verb', particles: ['particle'] };
        const result = sutra(sentence, context);
        expect(result.applies).toBe(true);
    });

    test('Handles particle found by substring match', () => {
        const sentence = "verb word particlecompound";
        const context = { isVedic: true, verb: 'verb', particles: ['particle'] };
        const result = sutra(sentence, context);
        expect(result.applies).toBe(true);
    });

    test('Handles particle not found in sentence', () => {
        const sentence = "verb word other";
        const context = { isVedic: true, verb: 'verb', particles: ['notfound'] };
        const result = sutra(sentence, context);
        expect(result.applies).toBe(false);
    });
