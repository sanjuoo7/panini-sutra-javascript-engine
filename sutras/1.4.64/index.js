/**
 * Sutra 1.4.64: भूषणेऽलम् (bhūṣaṇe'lam)
 * 
 * The word "alam" is classified as "gati" when used in the sense of
 * ornament/decoration (bhūṣaṇa) with a verb.
 * 
 * Traditional Commentary Integration:
 * - Kāśikā: अलमिति गतिसंज्ञा भवति भूषणार्थे प्रयुज्यमाने
 * - Patañjali: भूषणे अलम् इति गतिसंज्ञा भवति
 * 
 * Modern Explanation:
 * This sutra deals with the contextual classification of "alam" as गति 
 * when it expresses the sense of decoration or ornamentation in verbal constructions.
 * 
 * Linguistic Scope:
 * - Applies to: "alam" (अलम्) only
 * - Context requirement: Verbal context with meaning of ornament/decoration
 * - Classification: गति (indeclinable particles with pre-verbal function)
 * - Semantic constraint: Must express bhūṣaṇa (ornamentation)
 * 
 * Examples:
 * - अलम् करोति (alam karoti) - "decorates/ornaments" → alam = गति
 * - अलङ्करोति (alaṅkaroti) - formed compound meaning "to decorate"
 */
...
/**
 * Main function for Sutra 1.4.64: भूषणेऽलम्
 * 
 * Analyzes whether "alam" should be classified as गति when used
 * in contexts expressing ornamentation/decoration.
 * 
 * @param {string} word - The word to analyze (expecting "alam" or "अलम्")
 ...
 * @param {string} context.meaning - The contextual meaning ('ornament')
 * @returns {Object} Comprehensive analysis result
 */
function sutra164(word, context) {
  try {
    ...
    // Phase 4: Core Sutra Logic - Check if word is "alam"
    const wordAnalysis = analyzeTargetWord(workingWord);
    if (!wordAnalysis.isTargetWord) {
      return createNonApplicableResult(
        `Word "${normalizedWord}" is not "alam"`,
        { word: normalizedWord, expectedWords: ['alam', 'अलम्'] }
      );
    }
...
function validateContext(context) {
  ...
  if (!context.meaning) {
    return { 
      isValid: false, 
      reason: 'Contextual meaning required for bhūṣaṇa determination' 
    };
  }
...
/**
 * Analyzes if the word is "alam" in either script
 * @param {string} word - The word to analyze
 * @returns {Object} Word analysis result
 */
function analyzeTargetWord(word) {
  const alamWords = ['alam', 'अलम्'];

  if (alamWords.includes(word)) {
    return {
      isTargetWord: true,
      wordType: 'alam',
      iastForm: 'alam',
...
/**
 * Analyzes semantic context for bhūṣaṇa (ornamentation) determination
 * @param {Object} context - Context object
...
function analyzeSemanticContext(context) {
  ...
  if (meaning === 'ornament' || meaning === 'bhūṣaṇa') {
    return {
      applies: true,
      semanticType: 'bhūṣaṇa',
      reason: "Word 'alam' with meaning 'ornament' - expresses bhūṣaṇa",
      expectedMeaning: 'ornament'
    };
  } else {
    return {
      applies: false,
      reason: "Word 'alam' requires meaning 'ornament' for गति classification",
      expectedMeaning: 'ornament'
    };
  }
}
...
function createSuccessResult(originalWord, wordAnalysis, semanticAnalysis, context) {
  ...
  return {
    ...
    sutra: '1.4.64',
    sutraText: 'भूषणेऽलम्',
    translation: 'The word alam (is designated as gati) in the sense of ornamentation',
    
    // Morphological Analysis
    morphological: {
      category: 'gati',
      features: ['indeclinable'],
      subCategory: 'ornamental_qualifier',
    ...
    syntactic: {
      ...
      compoundPotential: 'alaṅkṛ (to decorate)'
    },
    ...
    reasons: [
      ...
      `Satisfies ${semanticAnalysis.semanticType} (ornamentation) requirement`,
      `Verbal context provided: ${context.verb}`,
      'Meets all conditions for गति classification per sutra 1.4.64'
    ],
    ...
    commentary: {
      kāśikā: 'अलमिति गतिसंज्ञा भवति भूषणार्थे प्रयुज्यमाने',
      translation: '"Alam" receives गति designation when used in the sense of ornamentation',
...
function getDetailedErrorMessage(errorType) {
  const messages = {
    ...
    'missing_meaning': 'Contextual meaning required for bhūṣaṇa determination',
...
function getErrorSuggestions(errorType) {
  const suggestions = {
    ...
    'missing_verb': [
      ...
      'Example: {verb: "kṛ", meaning: "ornament"}'
    ],
    'missing_meaning': [
      'Add meaning property to context object',
      'Use "ornament" for alam with bhūṣaṇa sense',
      'Meaning determines bhūṣaṇa classification'
    ]
  };
