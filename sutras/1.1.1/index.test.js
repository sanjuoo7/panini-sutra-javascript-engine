...
describe('Sutra 1.1.1: vṛddhirādaic (IAST)', () => {
  iastTestCases.forEach(({ word, vowel, expected }) => {
    test(`should return ${expected} for the vowel "${vowel}" in "${word}"`, () => {
      expect(isVrddhi(vowel)).toBe(expected);
    });
  });
});

describe('Sutra 1.1.1: vṛddhirādaic (Devanagari)', () => {
    devanagariTestCases.forEach(({ word, vowel, expected }) => {
        test(`should return ${expected} for the vowel "${vowel}" in "${word}"`, () => {
            expect(isVrddhi(vowel)).toBe(expected);
        });
    });
